...
const CreateQuestionForm = (props) => {
  ...
  const handleQuestionTypeChange = (e) => {
    ...
    if (value === 'tf') {
      ...
      reset({
        ...defaultValues,
        question_type: value,
        choices: [{ choice: 'True', correct_answer: false }, { choice: 'False', correct_answer: false }]
      });
    } else if (value === 'la') {
      setCount(1);
      reset({ ...defaultValues, question_type: value, choices: [{ choice: '', correct_answer: false }] });
    } else {
      setCount(3);
  ...
  const handleFormSubmit = async (data) => {
    const hasCorrectAnswer = data.choices.some(choice => choice.correct_answer);

    if (!hasCorrectAnswer) {
      console.log('No correct answer selected.');
    ...
    try {
      let response;
      if (Qid) {
        response = await EditQuestion({ guid: Qid, data });
      } else {
        response = await AddQuestion(guid, data);
      }
      setSubmitting(false);
      if (response.success) {
      ...
      {isLoading ? (
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)}>
          <Grid container spacing={5}>
            ...
            <Grid item xs={12}>
              {questionType == 'la' ? (<Typography sx={{ mb: 4 }}>Answer Details</Typography>) : (<Typography sx={{ mb: 4 }}>Answer Choices</Typography>)}

              {questionType === 'tf' ? (
                <FormControl component="fieldset">
                  <RadioGroup aria-label="true-false" name="true-false-options">
                    <FormControlLabel
                      value="true"
                      control={<Radio />}
                      label="True"
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <TextField
                        ...
                        {...field}
                        label='Long Answer'
                        error={Boolean(errors?.choices?.[0]?.choice)}
                        aria-describedby='validation-choice'
                      />
                    )}
                  />
                  {errors?.choices?.[0]?.choice && (
                    <FormHelperText sx={{ color: 'error.main' }} id='validation-choice'>
                      {errors.choices[0].choice.message}
                    </FormHelperText>
                  )}
                </FormControl>
              ) : (
              ...
              {questionType !== 'tf' && questionType !== 'la' && (

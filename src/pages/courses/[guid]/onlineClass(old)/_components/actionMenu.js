...
export default function ActionMenu({ id, itemId, onDeleteClick, onRemoveClick, onArchiveClick, guid }) {
  ...
  return (
    <div>
      ...
      >
        <Link href={`users?id=${id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
          <MenuItem onClick={handleClose}>
            <Icon icon='mdi:users' style={{ marginRight: '10px' }} />
            Users
          </MenuItem>
        </Link>
        <Link href={`edit?id=${id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
          <MenuItem onClick={handleClose}>
            <Icon icon='tabler:edit' style={{ marginRight: '10px' }} />
            Edit
          </MenuItem>
        </Link>
        <Link href={`share?id=${id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
          <MenuItem onClick={handleClose}>

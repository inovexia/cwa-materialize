...
const EditOnlineClasses = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (classguid, guid) {
        setLoading(true)
        const response = await ViewOnlineClass(classguid, guid)
        setLoading(false)
    ...
    fetchData()
  }, [classguid, guid, reset])

  /** EDIT ONLINE CLASS  */
  const onSubmit = async (data) => {
    if (classguid) {
      await EditOnlineClass(classguid, data)
        .then(response => {
  ...
  return (
    <>
      <Grid container spacing={6}>
        <Grid item xs={12}>
          <Card>
            <CardHeader title='Change date of online class' />
            <CardContent>
